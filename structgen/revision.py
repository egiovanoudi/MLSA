...
class RevisionDecoder(nn.Module):
    ...
    def __init__(self, args):
        ...
        # define loop specific attention layers
        self.attention_list = nn.ModuleList([self.loop_attention([filter[0], filter[1], filter[2]]) for _ in range(3)])
    ...
    def forward(self, true_X, true_S, L, mask, loop_length, context=None):
        B, N = mask.size(0), mask.size(1)
        v = []
        h_loops = []
        ...
        if self.context:
            for i in range(len(context)):
                h, cH, cmask, crange = self.encode_context(*context[i])
                h_loops.append(h)
            h = torch.cat(h_loops, dim=1)
        ...
        for t in range(N):
            ...
            v_loops = torch.split(V, loop_length, dim=1)
            for i in range(3):
                v.append(self.loop_specific(v_loops[i], i))      #loop specific attention
    ...
    def log_prob(self, true_S, mask, loop_length, context=None):
    ...
    def loop_attention(self, channel):
        att_block = nn.Sequential(
            nn.Conv1d(in_channels=channel[0], out_channels=channel[1], kernel_size=2, padding=0),
            nn.BatchNorm1d(channel[1]),
            nn.ReLU(inplace=True),
            nn.Conv1d(in_channels=channel[1], out_channels=channel[2], kernel_size=2, padding=1),
